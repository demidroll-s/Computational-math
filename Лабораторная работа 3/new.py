...
def plot_polar(ax, theta, r, values, levels, ylabel, title, savefig_name):
    ...
    cbar.ax.set_ylabel(ylabel, fontsize=14)
...
#-------------------------------------------------------------
values = [0 for m in range(M+1)]
for m in range(M+1):
    values[m] = [u[1][l][m] for l in range(L+1)]
...
plot_polar(ax, theta, r_, values, levels, ylabel, title, savefig_name)
...
for n in range(len(t)):
    for l in range(len(r)):
        for m in range(len(phi)):
            a = (r[l] * np.cos(phi[m]))**4 * (C1 - 2 * (mu + 2)/mu * t[n])**(-1/mu)
            u[n][l][m] = a
